...
function printUsage()
{
	...
	echo "    --packet-size        [number]    (optional) Min size of messages. e.g. 4B, 4KB, 4MB, default 4KB"
	...
	echo "    --no-throughput-test             (optional) Include this if you don't want to run the throughput test"
	echo "    --no-latency-test                (optional) Include this if you don't want to run the latency test"
...
java -cp ./lib/wantest.jar:./lib/portico/2.1.0/portico.jar wantest.Main $*
